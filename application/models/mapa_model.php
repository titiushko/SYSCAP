...
class Mapa_model extends CI_Model{
	...
	function cantidad_usuarios_centro_educativo($codigo_centro_educativo){
		$query = $this->db->query('SELECT SUM(capacitados.total) capacitados, SUM(CASE WHEN certificados.total IS NULL THEN 0 ELSE certificados.total END) certificados, (SUM(capacitados.total) + SUM(CASE WHEN certificados.total IS NULL THEN 0 ELSE certificados.total END)) total
								   FROM (SELECT nombre_municipio, total FROM V_UsuariosCapacitadosDepartamento WHERE id_centro_educativo = ?) capacitados
								   LEFT JOIN (SELECT nombre_municipio, total FROM V_UsuariosCertificadosDepartamento WHERE id_centro_educativo = ?) certificados
